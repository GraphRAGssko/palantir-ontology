...
export async function fetchInstances(
  ...
  const res = await fetch(`${OBJ}/${typeApiName}${qs}`);
  if (!res.ok) throw new Error("Failed to fetch instances");
  return res.json();
}

/** A single instance with its resolved links keyed by (inverse) api_name. */
export interface InstanceWithLinks extends Record<string, unknown> {
...
export async function fetchAudit(
  ...
  const res = await fetch(`${OBJ}/${typeApiName}/${encodeURIComponent(id)}/audit`);
