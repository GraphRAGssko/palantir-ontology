...
"""
Install hono, @hono/node-server, kysely, @cfworker/json-schema, and pg in apps/ontology
"""
...
"""
In apps/ontology/src, set up index.ts with a Hono app and a /health route, and db.ts with a Kysely connection reading DATABASE_URL from the workspace root .env. Add a dev script that runs the entry point with --watch.
"""
